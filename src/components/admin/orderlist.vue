                ...
                <table>
                    ...
                    <tbody>
                        <tr v-for="(order,index) in orderlists" :key="index">
                            ...
                            <td>
                                <button @click="delivergoods(order.order_id,index)" :disabled="order.order_status==1?false:((order.pay_channel=='货到付款'||order.pay_channel=='线下支付')&&order.order_status==0?false:true)">发货</button>
